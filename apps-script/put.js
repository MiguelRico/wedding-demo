...
function seedDemoData(data) {
  ...
  const sheetConfigs = {
    ...
    EscaletaMusical: [getMusicSongsSheet, MUSIC_SONGS_HEADERS],
  ...
  Object.keys(sheetConfigs).forEach((name) => {
    ...
    replaceSheetData(
      getSheet(),
      headers,
      entry.rows.map((row) => (Array.isArray(row) ? row : [])),
    );
  });
...
function saveConfirmation(data) {
  ...
  const keptConfirmationRows = confirmationRows.slice(1).filter((row) => {
    const matches =
      String(row[CONFIRMATIONS_COLUMNS.confirmationId] || "")
        .trim()
        .toLowerCase() === confirmationId;
    if (matches) existingConfirmationRow = row;
    return !matches;
  });
  const isExistingConfirmation = Boolean(existingConfirmationRow);
  confirmation.createdAt =
    existingConfirmationRow?.[CONFIRMATIONS_COLUMNS.createdAt] ||
    confirmation.createdAt;
  ...
  const existingGuestsById = getSheetRowsById(
    guestsSheet,
    GUESTS_COLUMNS.guestId,
  );
  const guestRows = guestsSheet
    .getDataRange()
    .getValues()
    .slice(1)
    .filter(
      (row) =>
        String(row[GUESTS_COLUMNS.confirmationId] || "")
          .trim()
          .toLowerCase() !== confirmationId,
    );
  guestsWithIds.forEach((guest) => {
    guest.createdAt =
      existingGuestsById[guest.guestId]?.[GUESTS_COLUMNS.createdAt] ||
      guest.createdAt;
    guestRows.push(buildGuestRow(confirmation, guest));
  });

  const assignmentRows = assignmentsSheet
    .getDataRange()
    .getValues()
    .slice(1)
    .filter(
      (row) =>
        String(row[TABLE_ASSIGNMENTS_COLUMNS.confirmationId] || "")
          .trim()
          .toLowerCase() !== confirmationId,
    );
  const assignmentContext = buildAssignmentContext();
  assignmentRows.push(
    ...buildAssignmentRowsForGuests(
      confirmation,
      guestsWithIds,
      assignmentContext,
    ),
...
function validateUniqueConfirmationContactFromRows(confirmation, rows) {
  ...
  rows.slice(1).forEach((row) => {
    ...
    if (
      email &&
      String(row[CONFIRMATIONS_COLUMNS.email] || "")
        .trim()
        .toLowerCase() === email
    )
    ...
    if (
      phone &&
      normalizePhoneSearch(row[CONFIRMATIONS_COLUMNS.phone]) === phone
    )
...
function saveTables(data) {
  ...
  (Array.isArray(data.tables) ? data.tables : []).forEach((table) => {
    ...
    tableRows.push([
      tableId,
      name,
      String(table.group || table.tag || "").trim(),
      String(table.tag || table.group || "").trim(),
      normalizeTableShape(table.shape),
      seatCount,
      table.notes || "",
      previousTables[tableId]?.[TABLES_COLUMNS.createdAt] ||
        table.createdAt ||
        now,
      now,
    ]);
    for (let index = 1; index <= seatCount; index++) {
      ...
      seatRows.push([
        ...
        tableId,
        String(index),
        previousSeats[seatId]?.[SEATS_COLUMNS.createdAt] ||
          table.createdAt ||
          now,
        now,
      ]);
  ...
  const assignmentRows = assignmentsSheet
    ...
    .filter(
      (row) =>
        validTableIds.has(
          String(row[TABLE_ASSIGNMENTS_COLUMNS.tableId] || "").trim(),
        ) &&
        validSeatIds.has(
          String(row[TABLE_ASSIGNMENTS_COLUMNS.seatId] || "").trim(),
        ),
    );
...
function saveProviders(data) {
  ...
  const previousProviders = getSheetRowsById(
    providersSheet,
    PROVIDERS_COLUMNS.providerId,
  );
  const previousServices = getSheetRowsById(
    servicesSheet,
    PROVIDER_SERVICES_COLUMNS.serviceId,
  );
  const previousPayments = getSheetRowsById(
    paymentsSheet,
    PROVIDER_PAYMENTS_COLUMNS.paymentId,
  );
  const providerRows = [],
    serviceRows = [],
    paymentRows = [];
  ...
  (Array.isArray(data.providers) ? data.providers : []).forEach((provider) => {
    ...
    providerRows.push([
      ...
      true,
      previousProviders[providerId]?.[PROVIDERS_COLUMNS.createdAt] ||
    ...
    (Array.isArray(provider.services) ? provider.services : []).forEach(
      (service) => {
        ...
        serviceRows.push([
          ...
          service.notes || "",
          true,
          previousServices[serviceId]?.[PROVIDER_SERVICES_COLUMNS.createdAt] ||
        ...
        (Array.isArray(service.payments) ? service.payments : [])
          ...
          .forEach((payment, index) => {
            ...
            paymentRows.push([
              ...
              payment.notes || "",
              previousPayments[paymentId]?.[
                PROVIDER_PAYMENTS_COLUMNS.createdAt
...
function saveNotifications(data) {
  ...
  const previous = getSheetRowsById(
    sheet,
    NOTIFICATIONS_COLUMNS.notificationId,
  );
  const rows = (
    ...
    return buildNotificationRow({
      ...notification,
      createdAt:
        previous[id]?.[NOTIFICATIONS_COLUMNS.createdAt] ||
...
function saveTasks(data) {
  ...
  const rows = (Array.isArray(data.tasks) ? data.tasks : []).map((task) => {
    ...
    return buildTaskRow({
      ...task,
      createdAt: previous[id]?.[TASKS_COLUMNS.createdAt] || task.createdAt,
    });
  });
...
function saveMusic(data) {
  ...
  const rows = (Array.isArray(data.music) ? data.music : []).map((song) => {
    ...
    return buildMusicSongRow({
      ...song,
      createdAt:
        previous[id]?.[MUSIC_SONGS_COLUMNS.createdAt] || song.createdAt,
    });
  });
  replaceSheetData(sheet, MUSIC_SONGS_HEADERS, rows);
  const momentsSheet = getMusicMomentsSheet();
  const previousMoments = getSheetRowsById(
    momentsSheet,
    MUSIC_MOMENTS_COLUMNS.momentId,
  );
  const momentRows = (Array.isArray(data.moments) ? data.moments : []).map(
    (moment) => {
      const id = String(moment.momentId || moment.id || "").trim();
      return buildMusicMomentRow({
        ...moment,
        createdAt:
          previousMoments[id]?.[MUSIC_MOMENTS_COLUMNS.createdAt] ||
          moment.createdAt,
      });
    },
  );
  ...
  const previousBlocks = getSheetRowsById(
    blocksSheet,
    MUSIC_BLOCKS_COLUMNS.musicBlockId,
  );
  const blockRows = (Array.isArray(data.blocks) ? data.blocks : []).map(
    (block) => {
      const id = String(block.musicBlockId || block.id || "").trim();
      return buildMusicBlockRow({
        ...block,
        createdAt:
          previousBlocks[id]?.[MUSIC_BLOCKS_COLUMNS.createdAt] ||
          block.createdAt,
      });
    },
  );
...
function updateNotificationRead(data) {
  ...
  for (let index = 1; index < rows.length; index++) {
    if (
      String(rows[index][NOTIFICATIONS_COLUMNS.notificationId] || "").trim() !==
      notificationId
    )
      continue;
    ...
    sheet
      .getRange(index + 1, 1, 1, NOTIFICATIONS_HEADERS.length)
